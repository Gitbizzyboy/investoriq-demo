#!/usr/bin/env python3
"""
InvestorIQ Property Intelligence Platform - Production Version
Complete investor platform with mapping, social intelligence, and business integration
"""
...
class PropertyIntelligencePlatform:
    ...
    def init_database_connections(self):
        """Initialize all database connections with relative paths"""
    ...
    def get_all_properties(self, filters=None):
        ...
        try:
            ...
            if not os.path.exists(self.master_db):
                print(f"Database not found: {self.master_db}")
                return []
            ...
            for row in cursor.fetchall():
                # Fix street view URL to work without API key
                original_street_view_url = row[13]
                # Use the property address as-is (it already includes city/state/zip)
                fixed_street_view_url = self.fix_street_view_url(row[2])
                
                # Also create alternative URLs for different mapping services
                google_maps_search = self.fix_street_view_url(row[2])
                apple_maps_url = self.create_apple_maps_url(row[2]) if row[2] else "#"
                
                # Clean up phone lookup URL (remove newlines and fix formatting)
                phone_url = row[12] if row[12] else "#"
                if phone_url and phone_url != "#":
                    # Remove newlines and extra spaces
                    phone_url = phone_url.replace('\n', '-').replace(' ', '-')
                    # Fix multiple dashes
                    phone_url = re.sub(r'-+', '-', phone_url)
                    # Ensure it starts with https://
                    if not phone_url.startswith('http'):
                        phone_url = f"https://www.whitepages.com/name/{phone_url}"
    ...
    def create_property_thumbnail(self, address, city, county):
        """Create actual street view image with multiple fallback options"""
        import urllib.parse
        import os
        ...
        address_short = address[:30] if address else "Property"
        return f"https://via.placeholder.com/400x300/4285f4/ffffff?text=📍+{urllib.parse.quote(address_short.replace(' ', '+'))}"
    ...
    def get_social_intelligence(self, city=None):
        """Get social media intelligence data"""
        try:
            if not os.path.exists(self.social_db):
                return []
                
            conn = sqlite3.connect(self.social_db)
            cursor = conn.cursor()
            ...
            conn.close()
            return social_data[:20]  # Return top 20 entries
            
        except Exception as e:
            print(f"Error getting social intelligence: {e}")
            return []
...
def index():
    ...
    return render_template('index.html', analytics=analytics, cities=cities)
...
@app.route('/api/social-intelligence')
def api_social_intelligence():
    """API endpoint for social intelligence data"""
    city = request.args.get('city')
    social_data = platform.get_social_intelligence(city)
...
if __name__ == '__main__':
    ...
    if os.path.exists('./QUAD_CITIES_MASTER_DATASET.db'):
        print("✅ Master database found")
    else:
