#!/usr/bin/env python3
"""
InvestorIQ Property Intelligence Platform - Admin Approval System
Professional platform with manual admin approval for new users
"""
...
class PropertyIntelligencePlatform:
    ...
    def init_users_database(self):
        """Initialize user authentication database with admin approval"""
        conn = sqlite3.connect(self.users_db)
        cursor = conn.cursor()
        
        # Update users table to include pending approval
        cursor.execute('''
    ...
    def create_user(self, email, password, first_name, last_name, company=None, phone=None, business_justification=None):
        """Create new user account with pending approval"""
        try:
            conn = sqlite3.connect(self.users_db)
            cursor = conn.cursor()
    ...
    def authenticate_user(self, email, password):
        """Authenticate user login - only approved users"""
        try:
            conn = sqlite3.connect(self.users_db)
            cursor = conn.cursor()
    ...
    # [Include all the property-related methods from the previous version]
...
def register():
    """User registration page - creates pending account"""
    if request.method == 'POST':
        email = request.form.get('email')
        password = request.form.get('password')
...
if __name__ == '__main__':
    ...
    print("🚀 InvestorIQ Property Intelligence Platform (Admin Approval)")
    print(f"🎯 Starting server on port {port}")
    print("🔒 Manual admin approval required for new users")
    print("👑 Admin panel available for user management")
